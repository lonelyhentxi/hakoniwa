...
export interface TrackingRegisterHTTPLoggerOptions<TL extends TrackingLogger<Cypress.WaitXHR>> extends Partial<Cypress.RouteOptions> {
  trackingLogger: TL;
}
...
export interface TrackingCheckHTTPLoggerOptions<Subject, TL extends TimelineTrackingLogger<Cypress.WaitXHR>> extends TrackingWaitHTTPLoggerOptions<Subject, TL> {
...
export interface TrackingAddHTTPLoggerOptions<TL extends TrackingLogger<Cypress.WaitXHR> = TrackingTimelineHTTPTrackingLogger<any>> extends Partial<Cypress.RouteOptions> {
  trackingLogger?: TL;
...
export type TrackingHTTPResPlainBody = {
  [key: string]: any;
  [index: number]: any;
} | Iterable<[any, any]> | Iterable<any>;

declare global {
  namespace Cypress {
    export type CyRouteOptions = RouteOptions;
    export interface Chainable<Subject> {
      trackingRegisterHTTPLogger<TL extends TrackingLogger<Cypress.WaitXHR>>(options: TrackingRegisterHTTPLoggerOptions<TL>): ReturnType<typeof cy.route>;
      ...
      trackingCheckHTTPLogger<TL extends TimelineTrackingLogger<Cypress.WaitXHR>>(options: TrackingCheckHTTPLoggerOptions<Subject, TL>): Chainable<TL>;
      trackingRegisterMethodLogger(options: TrackingRegisterMethodLoggerOptions): ReturnType<typeof cy.spy>;
    }
  }
}

Cypress.Commands.add('trackingRegisterHTTPLogger', (options: TrackingRegisterHTTPLoggerOptions<TrackingLogger<Cypress.WaitXHR>>) => {
...
Cypress.Commands.add('trackingWaitHTTPLogger', (options: TrackingWaitHTTPLoggerOptions<any, TimelineTrackingLogger<Cypress.WaitXHR>>) => {
  ...
  return cy.wrap(() => Date.now()).then(g => {
...
Cypress.Commands.add('trackingCheckHTTPLogger', (options: TrackingCheckHTTPLoggerOptions<any, TimelineTrackingLogger<Cypress.WaitXHR>>) => {
  const fromNow = !options.mark;
  return cy.wrap(() => Date.now()).then(g => {
    const config = Object.assign({
      mark: g()
...
export function trackingAddHTTPLogger<TL extends TrackingLogger<Cypress.WaitXHR> = TrackingTimelineHTTPTrackingLogger<any>>(options: TrackingAddHTTPLoggerOptions<TL> = {}) {
  const config = Object.assign({
