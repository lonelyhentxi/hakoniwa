...
export class JSONOp implements IProxyOp {
  ...
  op() {
    return `${this.protocol}://(${JSON.stringify(this.body)
      .replace(/ /g, '%20')
      .replace(/\r/g, '%0D')
      .replace(/\n/g, '%0A')})`;
  }
...
export class QueriesOp implements IProxyOp {
  ...
  op() {
    return `${this.protocol}://(${Object.keys(this.body)
...
export class FileOp implements IProxyOp {
  ...
  constructor(path: string, protocol = 'xfile', autocomplete = true) {
    this.path = path;
    this.protocol = protocol;
  ...
  op() {
    return this.autocomplete
      ? `${this.protocol}://${this.path.replace(/ /g, '%20')}`
      : `${this.protocol}://<${this.path.replace(/ /g, '%20')}>`;
  }
  toString() {
    return this.op();
